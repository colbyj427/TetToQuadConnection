...
class SurfaceMesh( object ):
    ...
    def GetFacesAdjacentToVertex( self, vertex_id ):
        ...
        while True:
            # Store the face if it is not on the boundary
            if iter_he.face != -1:
...
def PlotSurfaceMesh(mesh,face_values=[]):
...
def PlotBasic():    
    mesh=SurfaceMesh.FromOBJ_FileName("mixed_mesh_output100.obj")
    PlotSurfaceMeshIndexing(mesh)
    # PlotSurfaceMesh(mesh)
    # print(mesh.is_all_quad_mesh)
    # print(mesh.is_all_tri_mesh)
    # print(mesh.edges)
